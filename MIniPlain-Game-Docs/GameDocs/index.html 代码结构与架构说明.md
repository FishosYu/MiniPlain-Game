# MiniPlain.html 代码结构与架构说明

本文档从代码实现角度分析 `MiniPlain.html` 的结构和架构设计，适用于开发者理解代码组织方式。

## 1. 整体架构

`MiniPlain.html` 是一个单页 HTML 应用，所有代码（HTML、CSS、JavaScript）都在一个文件中。JavaScript 主体是一个立即执行函数表达式（IIFE），内部封装了所有游戏状态和逻辑，无外部依赖。
...
- 无需构建步骤，直接打开 `index.html`，再从简介页进入 `MiniPlain.html` 即可运行。
- 本项目根目录为 Godot 项目（`MiniPlain-Game`），`MiniPlain.html` 为快速原型/Demo 版本。
- Godot 版本计划在 `MIniPlain-Game-Docs/` 中的玩法文档和美术需求文档中有详细设计参考。

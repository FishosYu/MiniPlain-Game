# index.html 代码结构与架构说明

本文档从代码实现角度分析 `index.html` 的结构和架构设计，适用于开发者理解代码组织方式。

## 1. 整体架构

`index.html` 是一个单页 HTML 应用，所有代码（HTML、CSS、JavaScript）都在一个文件中。JavaScript 主体是一个立即执行函数表达式（IIFE），内部封装了所有游戏状态和逻辑，无外部依赖。
...
- 无需构建步骤，直接双击或用浏览器打开 `index.html` 即可运行。
- 本项目根目录为 Godot 项目（`MiniPlain-Game`），`index.html` 为快速原型/Demo 版本。
